...
router.post('/', async (req, res, next) => {
    try {
        ...
        if (includeMemory) {
            ...
            try {
                knowledgeContext = await knowledgeService.buildContextForQuery(userId, message, 5);
                logger.info(`为用户 ${userId} 加载知识库上下文`);
            } catch (knowError) {
                logger.warn('加载知识库上下文失败:', knowError.message);
            }
        }
        ...
        try {
            const dueReviews = await db.query(
                `SELECT m.id, m.content, m.category, m.importance 
                 ...
                 JOIN memories m ON rq.memory_id = m.id 
                 WHERE rq.user_id = ? AND rq.next_review <= NOW() AND rq.status = 'pending' 
            ...
            if (dueReviews && dueReviews.length > 0) {
                reviewContext = `\n\n【复习时间】以下记忆需要复习，请在对话中自然地提问帮助用户回忆（不要直接告诉答案）：\n${dueReviews.map((r, i) => `${i + 1}. [${r.category}] ${r.content.substring(0, 100)}...`).join('\n')}`;
                logger.info(`为用户 ${userId} 加载 ${dueReviews.length} 条复习内容`);
            }
        } catch (reviewError) {
        ...
        const contextParts = [];
        if (memoryContext) {
            contextParts.push(`以下是用户的相关记忆，请在回答时参考这些信息：\n${memoryContext}`);
        }
        if (knowledgeContext) {
            contextParts.push(`以下是用户的相关知识库内容，请在回答时参考这些信息：\n${knowledgeContext}`);
        }
...
4. 如果有复习内容，请在对话结束时自然地提问帮助用户复习`;
        ...
        res.json({
            ...
            data: {
                ...
                knowledgeLoaded: includeMemory && knowledgeContext.length > 0
            }
        });
    } catch (error) {
        next(error);
